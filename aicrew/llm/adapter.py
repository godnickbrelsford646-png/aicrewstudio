"""LLM adapter: real OpenAI-compatible provider + mock provider.

OpenAI and 302.ai gateway share the same wire format
...
Web search is NOT done here — it is performed by ``aicrew/tools/search.py``
(Tavily) BEFORE the LLM call. The executor prepends the search results to
the rendered prompt, so any LLM (openai, 302.ai, mock) sees the same
"WEB SEARCH RESULTS" block. This keeps the adapter simple and provider-agnostic.
